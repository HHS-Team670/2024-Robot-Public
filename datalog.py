...
if __name__ == "__main__":
    ...
    import mmap
    import sys
    fhand = open('shoot_data.csv', 'w')
    fhand.write("Timestamp,Angle,Distance\n")
    bad_subsystems = []
        ...
        for record in reader:
            ...
            if record.isStart():
                ...
                if entry.type == 'string':
                    if 'YELLOW' in record.getString() or 'RED' in record.getString() or 'Error' in record.getString() or 'CHANGED DRIVE CURRENT LIMIT' in record.getString() or entry.name in bad_subsystems:
                        lines = record.getString().split('\n')
                        for line in lines:
                            if 'YELLOW' in line or 'RED' in line or 'Error' in line or 'GREEN' in line or 'CHANGED DRIVE CURRENT LIMIT' in record.getString():
                                print(f"<name='{entry.name}', type='{entry.type}'> [{timestamp}] \n {line}")
                            if 'YELLOW' in line or 'RED' in line and entry.name not in bad_subsystems:
                                bad_subsystems.append(entry.name)
                        #print(record.getString())
                    elif 'Shot note' in record.getString():
                        lines = record.getString().split('\n')
                        for line in lines:
                            if 'Shot note' in line:
                                vals = line.split(' ')
                                fhand.write(str(timestamp) + ',' + str(vals[-6]) + ',' + str(vals[-1]) + '\n')
                                #print(line)
                elif entry.type == 'boolean':
                    if 'BrownedOut' in entry.name and record.getBoolean():
                        print(f"<name='{entry.name}', type='{entry.type}'> [{timestamp}] \n {record.getBoolean()}")
                continue
                print(f"<name='{entry.name}', type='{entry.type}'> [{timestamp}]")
